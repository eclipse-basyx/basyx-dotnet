...
namespace BaSyx.Models.Extensions
{
    ...
    public class ValueScopeConverter<TValueScope> : JsonConverter<ValueScope> where TValueScope : ValueScope
    {
        ...
        public static ValueScope ParseValueScope(SubmodelElement sme, JsonDocument value,
    JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            else if (sme.ModelType == ModelType.ReferenceElement)
            {
                ...
                {
                    Converters = { new ValueScopeConverter<ReferenceElementValue>(jsonOptions: jsonSerializerOptions) }
                });
            }
            else if (sme.ModelType == ModelType.BasicEventElement)
            {
                ...
                {
                    Converters = { new ValueScopeConverter<BasicEventElementValue>(jsonOptions: jsonSerializerOptions) }
                });
            }
            else if (sme.ModelType == ModelType.RelationshipElement)
            {
                ...
                {
                    Converters = { new ValueScopeConverter<RelationshipElementValue>(jsonOptions: jsonSerializerOptions) }
                });
            }
